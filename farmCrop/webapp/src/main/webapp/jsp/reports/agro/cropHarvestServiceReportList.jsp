...
<script type="text/javascript">
	$(document).ready(function(){
		jQuery("#detail").jqGrid(
				{
				   	url:'cropHarvestServiceReport_data.action',
				    ...
				    styleUI : 'Bootstrap',
							...
							'<s:text name="app.subBranch"/>',    
				  		    ...
				  		    '<s:text name="cSeasonCode"/>',
				  		   	 '<s:text name="village"/>',
				  		   	  ...
				  		   	  /* '<s:text name="totalYieldPrice"/> (<s:property value="%{getCurrencyType().toUpperCase()}" />)' */
			           ...
			           <s:if test='isMultiBranch=="1"&&(getIsParentBranch()==1||branchId==null)'>
			   			{name:'subBranchId',index:'subBranchId',width:125,sortable: false,width :125,search:true,frozen:true,stype: 'select',searchoptions: { value: '<s:property value="childBranchFilterText"/>' }},	   				   		
								...
								{name:'totalQuantity',index:'totalQuantity',width:250,sortable:false,align:'right',search:false}
						   		/* {name:'totalYieldPrice',index:'totalYieldPrice',width:250,sortable:false,align:'right',search:false} */
...
<sec:authorize ifAllGranted="service.report.cropHarvestReport.create">
...
<s:form name="createform" action="cropHarvestServiceReport_create">
</s:form>
<s:form name="updateform" action="cropHarvestServiceReport_detail">
	<s:hidden key="id" />
