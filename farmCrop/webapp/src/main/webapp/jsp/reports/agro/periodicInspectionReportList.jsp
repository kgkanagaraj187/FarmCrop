...
function playAudioFiles(id, inspectionDate, farmerIdName, farmIdName){
	...
	$("#farmData").html(farmIdName);
	
	$("#jquery_jplayer_1").jPlayer({
		ready: function () {
			$(this).jPlayer("setMedia", {
				mp3: "periodicInspectionReport_populateAudioPlay?id="+id+"&dt="+new Date()
			}).jPlayer("play");
		},
		swfPath: "js/jplayer",
		solution:"html,flash",
...
function icsNameShow(){
	if('<s:property value="getCurrentTenantId()"/>'=='pratibha'){
	jQuery("#icsNameField").show();
	}
	else
		jQuery("#icsNameField").hide();
}
...
function loadRegularGrid(){
		jQuery("#detail").jqGrid(
			{
			   	url:'periodicInspectionReport_data.action',
...
'<s:text name="app.subBranch"/>',
			  		    ...
			  		    <s:if test="currentTenantId!='efk'">
			  		      '<s:property value="%{getLocaleProperty('father')}"/>', 
			  		      </s:if>
			  		      <s:else>
						  ...
						  <s:if test="currentTenantId!='pratibha'&& currentTenantId!='chetna' && currentTenantId!='indev' && currentTenantId!='crsdemo' && currentTenantId!='iccoa' &&  currentTenantId!='agro'">
			      	...
			      	colModel:[
						...
						{name:'inspectionDate',index:'inspectionDate',sortable:false,frozen:true},
						{name:'seasonCode',index:'seasonCode',sortable:false,frozen:true},
						{name:'farmerIdName',index:'farmerIdName',sortable:false,frozen:true},
			    ...
			    beforeSelectRow: 
			    	...
			    	var branchId = '<s:property value="getBranchId()"/>';
			            if(branchId==null||branchId==""){
			            	if (iCol ==3 || iCol ==12 || iCol ==13 || iCol ==11 ){return false; }
					            else{ return true; }
			            }else{
			            	 if (iCol ==2 || iCol ==11 || iCol ==12 || iCol ==12){return false; }
					            else{ return true; }
			            }
...
'<s:text name="app.subBranch"/>',
		     	...
		     	colModel:[
				    ...
				    {name:'father',index:'father',sortable:false},
					{name:'farmId',index:'farmId',sortable:false},
					{name:'cropCode',index:'cropCode',sortable:false},
					<s:if test="currentTenantId=='pratibha' && branchId!='bci'">
					{name:'icsName',index:'icsName',sortable:false},
		    ...
		    onSelectRow: function(id){ 
    			     ...
    			     getSelectUpdateData();
					 var postdataReport =  JSON.stringify($('#needDetail').getGridParam('postData'));
		   ...
		   beforeSelectRow: 
					    	...
					    	var branchId = '<s:property value="getBranchId()"/>';
					            if(branchId==null||branchId==""){
					            	if (iCol ==3 || iCol ==10 || iCol ==11 ){return false; }
							            else{ return true; }
					            }else{
					            	 if (iCol ==2 || iCol ==9 || iCol ==10){return false; }
							            else{ return true; }
					            }
...
function exportXLS(inspType){
	
	if(isDataAvailable("#"+inspType)){
		jQuery("#"+inspType).setGridParam ({postData: {rows : 0}});
		jQuery("#"+inspType).jqGrid('excelExport', {url: 'periodicInspectionReport_populateXLS.action'});
	}else{
...
<body>
	...
	<s:form name="form" id="filter" action="periodicInspectionReport_list">
		<div class="appContentWrapper marginBottom">
			<section class='reportWrap row'>
				<div class="gridly">
					...
					list="farmerFartherNameList" headerKey=""
					...
					<s:if
						...
						<div class="reportFilterItem">
							<label for="filterEle"><s:text name="app.subBranch" /></label>
							<div class="form-element">
				    ...
				    <s:if test="currentTenantId=='pratibha' && branchId=='bci'">
				  		  ...
				  		  <div class="filterEle hide" id="icsNameField">
						<label for="txt"><s:text name="icsName" /></label>
						<div class="form-element">
							<s:select name="icsName" id="icsName"
								class="form-control input-sm select2" list="{}"
								headerKey="" headerValue="%{getText('txt.select')}"
					...
					<s:else>
					  <div class="filterEle" id="icsNameField">
						<label for="txt"><s:text name="icsName" /></label>
						<div class="form-element">
						<s:select name="icsName" id="icsName"
								class="form-control input-sm select2" list="icsStatusList"
								headerKey="" headerValue="%{getText('txt.select')}"
								listKey="key" listValue="value"></s:select>
						</div>
						</div>
	...
	<div class="tab-content ">
		<div id="tabs-1" class="tab-pane fade in active">
			<div class="appContentWrapper marginBottom">
				<input type="BUTTON" class="btn btn-sts pull-right marginBottom"
					value="<s:property
								value="%{getLocaleProperty('exportXLS')}" />" onclick="exportXLS('detail');"
					title="Export XLS" />
				<div style="width: 99%;margin-top: 2%" id="baseDiv">
					<br>
		...
		<div id="tabs-2" class="tab-pane fade">
			<div class="appContentWrapper marginBottom">
				<input type="BUTTON" class="btn btn-sts pull-right"
					value="<s:property
								value="%{getLocaleProperty('exportXLS')}" />"
					onclick="exportXLS('needDetail');" title="Export XLS" />
				<div style="width: 99%;margin-top: 2%" id="baseDiv">
					<br>
	...
	<s:form name="detailform" action="periodicInspectionReport_detail">
		<s:hidden key="id" />
	...
	<s:form name="updateform" action="periodicInspectionReport_detailNeed">
		<s:hidden key="id" />
		<s:hidden name="tabIndex" />
		<s:hidden key="currentPage" />
		<s:hidden name="filterMapReport" id="filterMapReportUpdate" />
		<s:hidden name="postdataReport" id="postdataReportUpdate" />
	</s:form>

	<s:form id="audioFileDownload"
		action="periodicInspectionReport_populateDownload">
