...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		jQuery("#detail").jqGrid(
				{
					url:'offlineProcurementReport_detail.action?',
				   	...
				   	colModel:[
						...
						{name:'receiptNo',index:'receiptNo',width:250,sortable:false},
				   		...
				   		{name:'statusMsg',index:'statusMsg',width:250,sortable:false,align:'right'}
				   	],
				   ...
				   subGridRowExpanded: function(subgrid_id, row_id){
					   ...
					   {
							url:'offlineProcurementReport_subGridDetail.action?id='+row_id,
						   	pager: pager_id,
						   	datatype: "json",	
						   	colNames:[	
									  '<s:text name="productCode"/>',
									  '<s:text name="quality"/>',
									  '<s:text name="numberOfBags"/>',
									  '<s:text name="grossWeight"/>',
									  '<s:text name="tareWeight"/>',
						  		   	  '<s:text name="netWeight"/>',
						  		   	  '<s:text name="ratePerUnit"/>',			  		  	 
						  		   	  '<s:text name="subTotal"/>'
				 	      	 ],
						   	colModel:[	
									{name:'productCode',index:'productCode',width:150,sortable:false},
									{name:'quality',index:'quality',width:150,sortable:false},
									{name:'numberOfBags',index:'numberOfBags',width:150,sortable:false,align:'right'},
									{name:'grossWeight',index:'grossWeight',width:150,sortable:false,align:'right'},
									{name:'tareWeight',index:'tareWeight',width:150,sortable:false,align:'right'},
		   							{name:'netWeight',index:'netWeight',width:150,sortable:false,align:'right'},
		...
		function reloadGrid(){
			...
			if (startDate>endDate){
				...
				jQuery("#detail").jqGrid('setGridParam',{url:"offlineProcurementReport_detail.action?",page:1}).trigger('reloadGrid');
			}	
		}
		...
		function popupWindowForReconcile(id) { 
			 if(confirm( '<s:text name="status.change"/>')){
				 $.post('offlineProcurementReport_reconcile.action', {'id':id},
				      function() {
				    	reloadGrid();
			});
			}
	...
	function exportXLS(){
		if(isDataAvailable("#detail")){
			...
			jQuery("#detail").jqGrid('excelExport', {url: 'offlineProcurementReport_populateXLS.action?'});
		}else{
...
<s:text name="filterSelection" /></font></legend> <s:form name="form" action="offlineProcurementReport_list"  style="width:600px; float:left;">
	...
	<table>
		<tr>
			<td style="width: 120px"><s:text name="type" /></td>
			<td><s:select name="filter.type" theme="simple" list="typeList"
				headerKey="" headerValue="%{getText('txt.select')}" listKey="key" listValue="value"
				theme="simple" id="type" cssStyle="width:200px;margin:5px 0 5px 0;" />
			</td>
		</tr>
	</table>
	</div>
