						...
						function() {
						    ...
						    if($(this).attr("class").indexOf("details-control")<0){
						        ...
						        var fikter=containsAny(flt,$(this).attr("class").split(" "));
						         var tesst =$('<input type="text" placeholder="Search '+title+'" id="'+i+'" />' );
						       var closeBtn = $('<a title="Reset Search Value" style="padding-right: 0.3em;padding-left: 0.3em;color:white" class="clearsearchclass">x</a>');
						        ...
						        //$(this).html('');
						         $(closeBtn).on( 'click', function () {
						        	 $( this).parent().parent().find('.filt').val('');
						        	 if(  $(this).hasClass("4")) {
						         		 min = '';
									    	dataInd =i; 
									    	max = '';
					...
					.DataTable(
							{
								...
								var api = this.api();
								var footCol = '<s:property value="footerSumCols"/>';
								footCol = footCol.trim().split("#").sort();
								if ('<s:property value="subGridCols"/>' != null
										&& '<s:property value="subGridCols"/>' != '') {
								$('#foot'+(parseInt(footCol[0])-2)).html("TOTAL");
								$(footCol).each(
										function(k, val) {
												$('#foot'+val)
													.html(api.column($('#foot'+val).index(),{ search:'applied' }, {
														page : 'all'
													}).data().sum().toFixed(2));
										});
								}else{
									$('#foot'+(parseInt(footCol[0])-1)).html("TOTAL");
									$(footCol).each(
											function(k, val) {
													$('#foot'+val)
														.html(api.column($('#foot'+val).index(),{ search:'applied' }, {
								...
								dom : 'Bfrtip',
								 lengthMenu: [
								              ...
								              url: '//cdn.datatables.net/plug-ins/1.10.15/i18n/Bangla.json'
								          }, */
								buttons : [
'pageLength','colvis' ,
										{
											extend : 'excelHtml5',
											filename :$('.breadCrumbNavigation li').last().text().replace(/\s/g, ''),
											title : $('.breadCrumbNavigation li').last().text().replace(/\s/g, ''),
											footer: true,
											 customizeData: function ( data ) {
												 var header=[];
												 $('#detail thead tr:eq(0) th:not(.details-control)').each( function (i) {
										...
										},
										{
						                    ...
						                    /* action: function(e, dt, button, config) {
						                    	document.pdf.pdfData.value =$('#detail').html();
						                    	document.pdf.pdfFileName.value =$('.breadCrumbNavigation li').last().text().replace(/\s/g, '');
						                    	document.pdf.action="dynamicViewReportDT_populatePDF.action";
						                    	document.pdf.submit();
												 ...
												 $('#detail thead tr:eq(0) th:not(.details-control)').each( function (i) {
													 var dat={};
													 dat['text'] = $(this).clone().children().remove().end().text();
													 dat['style'] = 'tableHeader';
													 header.push(dat);
												 });
												 data.content[1].table.body[0] = header;
										...
										},
										{
						                    ...
						                    fieldBoundary: '',
						                    customize: function ( data ) {
										/*if you change the fieldSeparator change here also*/
													var splitted = data.split("\n");     
											var header = 	$.map( $('#detail thead tr:eq(0) th:not(.details-control)'), 
														  function(element) {
														      return $(element).clone().children().remove().end().text();
														  })
														  .join(";");
											data = data.replace(splitted[0],header);
												return data;
											 },
										...
										},
										{
											...
											exportOptions: {
						                    	...
						                    	var tr = $('#detail thead tr:eq(0)').clone();
						                    	 $(this).find('th.details-control').remove();
						                    	 $(tr).find('th:not(.details-control)' ).each( function (i) {
						                    		
						                    		 $(this).find('td.filterTd').remove();
						                    	
						                    	 });
						                    	 $(win.document.body).find( 'table thead tr:eq(0)' ).replaceWith(tr);
						                    	 return win;
										 },
										...
										},
										{
											...
											customize: function ( data ) {
												...
												var header = 	$.map( $('#detail thead tr:eq(0) th:not(.details-control)'), 
															  ...
															  .join(" ");
												data = data.replace(splitted[2],header);
													return data;
										 },
