...
<body>
	...
	function loadGrid(){
		...
		$(subColNames.split("%")).each(function(k,val){
			if(!isEmpty(val)){	
				...
				subGridColumnModels.push({name: cols[0],sortable: false,frozen:true});
			}
		});
		...
		jQuery("#detail").jqGrid(
				{
					url:'agentProductReturnReport_detail.action?',
					   ...
					   jQuery("#"+subgrid_table_id).jqGrid({
						   url:'agentProductReturnReport_subGridDetail.action?id='+row_id,
						   pager: pager_id,
						   datatype: "json",	
	...
	function exportXLS(){
		if(isDataAvailable("#detail")){
			...
			$("#form select").each(function(){
				var value = $(this).val()+"~"+ $(this).children(":selected").text();
				var name=this.name;
				if(!isEmpty(value)){
			...
			jQuery("#detail").jqGrid('excelExport', {url: 'agentProductReturnReport_populateXLS.action?',postData:dataa});
		}else{
	...
	function search(){
		...
		jQuery("#detail").jqGrid('setGridParam',{url:'agentProductReturnReport_detail.action',page:1,postData:dataa}).trigger('reloadGrid');				
		}	
