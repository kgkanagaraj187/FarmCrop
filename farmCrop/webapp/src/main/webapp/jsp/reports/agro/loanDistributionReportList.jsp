...
<body>
	...
	function loadGrid(){
		...
		jQuery("#detail").jqGrid(
				{
					url:'loanDistributionReport_detail.action?',
					 ...
					 postData:{				 
				   	...
				   	colNames:[
				   	   '<s:property value="%{getLocaleProperty('Loan Date')}" />' ,				   	  
				   		'<s:property value="%{getLocaleProperty('Loan Acc No')}" />' ,
				    	//'<s:property value="%{getLocaleProperty('loanTo')}" />',
				    	'<s:property value="%{getLocaleProperty('Vendor')}" />',				    					
				    	'<s:property value="%{getLocaleProperty('Farmer Name')}" />',						
						//'<s:property value="%{getLocaleProperty('village.name')}" />',				  		  	
			  		  	//'<s:property value="%{getLocaleProperty('loanCategory')}" />',
			  		  	'<s:property value="%{getLocaleProperty('totalQty')}" />' ,
			  		  	'<s:property value="%{getLocaleProperty('Total Cost')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	  	
				  		'<s:property value="%{getLocaleProperty('interestPercentage')}" />' ,
				  		'<s:property value="%{getLocaleProperty('interestPercentageAmt')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	 
				  		'<s:property value="%{getLocaleProperty('loanTenureLabel')}" />',				  		  	
			  		    '<s:property value="%{getLocaleProperty('loanTenureAmt')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	 
			  		  	'<s:property value="%{getLocaleProperty('totalCost')}" />'+"("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	 
				  		'<s:property value="%{getLocaleProperty('monthlyRepaymentAmt')}" />' +" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")" 
				   	...
				   	colModel:[
						...
						//{name:'loanTo',index:'loanTo',width:250,sortable:false},
						{name:'vendor',index:'vendor',width:250,sortable:false},
						{name:'farmerName',index:'farmerName',width:350,sortable:false},
						//{name:'v.name',index:'v.name',width:350,sortable:false},
						//{name:'loanCategory',index:'loanCategory',width:350,sortable:false},
						{name:'totalQty',index:'totalQty',width:350,sortable:false,align:'right'},
						{name:'costToFarmer',index:'costToFarmer',width:350,sortable:false,align:'right'},
						{name:'interestPercentage',index:'interestPercentage',width:350,sortable:false,align:'right'},
						{name:'interestPercentageAmt',index:'interestPercentageAmt',width:350,sortable:false,align:'right'},
						{name:'loanTenure',index:'loanTenure',width:300,sortable:false,align:'right'},
				   		{name:'loanTenureAmt',index:'loanTenureAmt',width:300,sortable:false,align:'right'},				   		
				   		{name:'totalCost',index:'totalCost',width:300,sortable:false,align:'right'},
				   		{name:'monthlyRepaymentAmt',index:'monthlyRepaymentAmt',width:400,sortable:false,align:'right'},
				   ...
				   subGridRowExpanded: function(subgrid_id, row_id){
					   ...
					   {
							url:'loanDistributionReport_subGridDetail.action?id='+row_id,
						   	pager: pager_id,
						   	datatype: "json",	
						   	colNames:[
						   		'<s:property value="%{getLocaleProperty('distributionBalance.subCategory')}" />',
						   	    '<s:property value="%{getLocaleProperty('productName')}" />',
						   	  	'<s:property value="%{getLocaleProperty('ratePerUnit')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	  	
						   	 	'<s:property value="%{getLocaleProperty('quantity')}" />',
						  		'<s:property value="%{getLocaleProperty('amount')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	  	
						  		//'<s:property value="%{getLocaleProperty('subsidyInterest')}" />',
							  	//'<s:property value="%{getLocaleProperty('subsidyAmt')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	  	
						  		'<s:property value="%{getLocaleProperty('totalAmt')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	  	
						   	...
						   	colModel:[	
						   		{name:'category',index:'category',width:150,sortable:false},
									...
									{name:'ratePerUnit',index:'ratePerUnit',width:150,sortable:false,align:'right'},
		   							{name:'quantity',index:'quantity',width:150,sortable:false,align:'right'},
		   							{name:'amount',index:'amount',width:100,sortable:false,align:'right'},		   							
		   							//{name:'subsidyInterest',index:'subsidyInterest',width:150,sortable:false},
		   							//{name:'subsidyAmt',index:'subsidyAmt',width:150,sortable:false,align:'right'},		   							
	...
	function exportXLS(){
		if(isDataAvailable("#detail")){
			...
			jQuery("#detail").jqGrid('excelExport', {url: 'loanDistributionReport_populateXLS.action?',postData:dataa});
		}else{
	...
	function search(){
		...
		loadFields();
		jQuery("#detail").jqGrid('setGridParam',{url:'loanDistributionReport_detail.action',page:1,postData:dataa}).trigger('reloadGrid');				
		}	
function loadFields(){
	  ...
	  $.post("loanDistribution_populateHeaderFields",{filterList:filterDatas,startDate:startDate,endDate:endDate,branch:branchId},function(data){
		...
		<div class="appContentWrapper marginBottom">
			<section class='reportWrap row'>
				...
				<div class="filterEle">
				 ...
				 <div class="filterEle">
						<label for="txt"><s:text name="procFarmer" /></label>
						<div class="form-element">
					...
					<%-- <div class="filterEle">
						...
						<div class="form-element">
							<s:select name="filter.agroTransaction.farmerName" id="groupId" list="groupList"
								headerKey="" headerValue="%{getText('txt.select')}"
								class="form-control input-sm select2" />
						</div>
					</div> --%>
					<div class="filterEle hide">
					<label for="txt"><s:text name="City/Town" /></label>
					<div class="form-element">
						<s:select name="filter.village.name" id="villageName" list="villageMap"
							headerKey="" headerValue="%{getText('txt.select')}"
							class="form-control input-sm select2" />
					</div>
				</div> 
