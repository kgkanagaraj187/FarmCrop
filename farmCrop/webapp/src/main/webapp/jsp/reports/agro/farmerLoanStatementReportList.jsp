...
<body>
	...
	function loadGrid(){
		...
		jQuery("#detail").jqGrid(
				{
					url:'farmerLoanStatementReport_detail.action?',
					...
					postData:{				 
				   	...
				   	colNames:[
						...
						'<s:property value="%{getLocaleProperty('villageName')}" />',				  		  	
			  		  	'<s:property value="%{getLocaleProperty('loanAmt')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	
			  		  	'<s:property value="%{getLocaleProperty('loanRepayment')}" />' +" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")",	
			  		  	'<s:property value="%{getLocaleProperty('loanBalance')}" />'+" ("+'<s:property value="%{getCurrencyType().toUpperCase()}" />'+")"  	
				   	...
				   	colModel:[
						...
						{name:'farmerCode',index:'farmerCode',width:250,sortable:false},
						{name:'v.name',index:'v.name',width:300,sortable:false},					
						{name:'loanAmt',index:'loanAmt',width:300,sortable:false,align:'right'},
				   		{name:'loanRepayment',index:'loanRepayment',width:300,sortable:false,align:'right'},
				   		{name:'loanBalance',index:'loanBalance',width:300,sortable:false,align:'right'}
				   ...
				   subGridRowExpanded: function(subgrid_id, row_id){
					   ...
					   {
							url:'farmerLoanStatementReport_subList.action?id='+row_id,
						   	pager: pager_id,
						   	datatype: "json",	
						   	colNames:[	
	...
	function exportXLS(){
		if(isDataAvailable("#detail")){
			...
			jQuery("#detail").jqGrid('excelExport', {url: 'farmerLoanStatementReport_populateXLS.action?',postData:dataa});
		}else{
	...
	function search(){
		...
		//loadFields();
		jQuery("#detail").jqGrid('setGridParam',{url:'farmerLoanStatementReport_detail.action',page:1,postData:dataa}).trigger('reloadGrid');				
		}	
function loadFields(){
	  ...
	  $.post("farmerLoanStatementReport_populateHeaderFields",{filterList:filterDatas,startDate:startDate,endDate:endDate,branch:branchId},function(data){
