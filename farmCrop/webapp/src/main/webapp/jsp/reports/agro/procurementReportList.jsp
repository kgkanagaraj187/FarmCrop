...
var enableSupplier = "<s:property value='supplierEnabled'/>";
//alert(enableSupplier);
	...
	jQuery(document).ready(function(){
		...
		loadCustomPopup();
		loadFields();
		if(tenantId=="kpf"||tenantId=="KPF"||tenantId=="simfed"||tenantId=="wub"||tenantId=="gsma"){
			var columnNames ;
			loadKPFGrid();
		}else{
	...
	function loadGrid(){
		...
		$(colNames.split("%")).each(function(k,val){
				if(!isEmpty(val)){
					...
					gridColumnModels.push({name: cols[0],align: cols[2],sortable: false,frozen:true, align:'right'});
					}
				}
		});
...
<s:if test="currentTenantId!='gsma'">
				   	 ...
				   	 '<s:property value="%{getLocaleProperty('txnDate')}" />' ,
				   	</s:else>
			  		  ...
			  		  <s:if test="currentTenantId!='kpf'&& currentTenantId!='simfed'&& currentTenantId!='wub'&& currentTenantId!='gsma'">
				   	...
				   	colModel:[
						...
						{name:'txnDate',index:'txnDate',width:250,sortable:false},
	...
	function exportXLS(){
		if(isDataAvailable("#detail")){
			...
			headerData='<s:property value='%{getLocaleProperty("totalNetwt")}'/>'+"###"+totalNetwt+"###"+'<s:text name="txnAmount"/> (<s:property value="%{getCurrencyType().toUpperCase()}" />)'+"###"+txnAmount+"###"+'<s:text name="txnAmt"/> (<s:property value="%{getCurrencyType().toUpperCase()}" />)'+"###"+totalPayAmt;
	...
	function enablePrinter(farmerId){
		 window.open("procurementReport_populatePrintz.action?selectedProcId="+farmerId.toString(), '_blank');
	}
	 ...
	 //	getChartFieldData(filterDatas,weightUnit,drillDownMsg,drilldownTooltip1,drilldownTooltip2,barChartLegend,barChartLegendTxnAmt,barChartLegendPayAmt);	 	
		...
		<div class="appContentWrapper marginBottom">
			<section class='reportWrap row'>
				...
				<s:if test="tenantId=='kpf' || tenantId=='simfed' || tenantId=='wub' || tenantId=='gsma'">
					<div class="filterEle">
			...
			<div class="flexItem-2">
				<div class="summaryBlocksWrapper flex-container ">
					<div class="report-summaryBlockItem">
						<span> <s:property value="%{getLocaleProperty('totalNetwt')}"/>&nbsp;<span class="strong" id="totalNetwt"></span><s:if test="currentTenantId!='pratibha'">&nbsp;(kgs)</s:if></span>
					</div>
					<div class="report-summaryBlockItem">
						<span><s:text name="txnAmount"/>&nbsp;<span class="strong" id="txnAmount"></span>&nbsp; <s:if test="currentTenantId=='pratibha'">(<s:property value="%{getLocaleProperty('priceUnit')}"/>) </s:if><s:else>(<s:property value="%{getCurrencyType().toUpperCase()}" />)</s:else></span>
					</div>
					<div class="report-summaryBlockItem">
						<span><s:text name="txnAmt"/>&nbsp;<span class="strong" id="totalPayAmt"></span>&nbsp; <s:if test="currentTenantId=='pratibha'">(<s:property value="%{getLocaleProperty('priceUnit')}"/>) </s:if><s:else>(<s:property value="%{getCurrencyType().toUpperCase()}" />)</s:else></span>
					</div>
