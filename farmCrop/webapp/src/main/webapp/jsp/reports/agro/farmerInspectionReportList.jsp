...
<body>
	...
	jQuery(document).ready(function(){
		...
		loadCustomPopup();
		getFilterData();
		var typezz=<%out.print("'"+request.getParameter("type")+"'");%>
		jQuery(".well").hide();
		...
		jQuery("#detail").jqGrid(
				{
				   	url:'farmerInspectionReport_data.action?',
					...
					colNames:[	
						...
						<s:if test='isMultiBranch=="1"&&(getIsParentBranch()==1||branchId==null)'>
					   	   ...
					   	   '<s:text name="%{getLocaleProperty('farmer')}"/>',
					   		  '<s:text name="farmIdCode"/>',
					   		  '<s:text name="farmNames"/>',
		...
		function reloadGrid(){var d1=	jQuery('#daterange').val();
		var d2= d1.split("-");
		//	alert(d1);
		var value1= d2[0];
		//alert(value1);
	...
	//	alert(endDate);
		...
		jQuery("#detail").jqGrid('setGridParam',{url:"farmerInspectionReport_data.action?",page:1})
			.trigger('reloadGrid');	
	...
	function exportXLS() {
	    if (isDataAvailable("#detail")) {
	        ...
	        jQuery("#detail").jqGrid('excelExport', {url: 'farmerInspectionReport_populateXLS.action?'});
	}     else {
	        alert('<s:text name="export.nodata"/>');
	    }
	}
	function onFilterData(){
		callAjaxMethod("farmerInspectionReport_populateFarmerList.action","farmerId");
		callAjaxMethod("farmerInspectionReport_populateFarmList.action","farmId");
	}
	...
	<s:form name="form" id="filter" action="farmerInspectionReport_list">
		<div class="appContentWrapper marginBottom">
			<section class='reportWrap row'>
				<div class="gridly">
			<div class="filterEle">
					<label for=""><s:text name="startDate" /></label>
				...
				<div class="filterEle">
					<label><s:text name="farmer" /></label>
					<div class="form-element">
		...
		<s:if test='branchId==null'>
				<div class="filterEle">
					<label><s:text name="app.branch" /></label>
					<div class="form-element">
	...
	<%-- <s:hidden name="type" class="paramType"></s:hidden> --%>
	<s:form name="updateform" action="farmerInspectionReport_detail">
		<s:hidden key="id" />
