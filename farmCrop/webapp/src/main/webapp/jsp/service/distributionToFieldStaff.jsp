...
<s:form name="form" cssClass="fillform"
	action="distributionToFieldStaff_%{command}">
				...
				<div class="flexiWrapper filterControls">
					...
					<div class="flexi flexi10">
						<label for="txt"><s:property
								value="%{getLocaleProperty('warehouse.name')}" /><span
							class="manadatory">*</span></label>
						<div class="form-element">
							<s:if
								test="currentTenantId!='lalteer' && currentTenantId!='pratibha'">
							...
							<s:elseif test="currentTenantId=='lalteer'">
								<s:select name="selectedWarehouse" id="warehouse"
									list="coopearativeList"
							...
							<s:else>
								<s:select name="selectedWarehouse" id="warehouse"
									list="coopearativeList"
									onchange="populateMobileUser();populateSubCategory();resetWarehouseData();resetPrefixAndSuffix();"
									headerKey="" headerValue="%{getText('txt.select')}"
									cssClass="form-control input-sm select2" listKey="key"
									listValue="value" />
							</s:else>
					...
					<div class="flexi flexi10">
						...
						<div class="form-element">
							<s:textfield name="startDate" id="startDate" readonly="true"
								theme="simple" size="20" data-date-viewmode="years"
				...
				<div class="appContentWrapper marginBottom">
					<div class="formContainerWrapper">
						...
						<div class="flexiWrapper filterControls">
							...
							<div class="flexi flexi10" style="width: 230px">
								...
								<div class="form-element">
									<div class="button-group-container">
										<s:textfield id="distributionStock" name="distributionStock"
											...
											onkeyup="calulateAvailableStock();" cssStyle="width:100%" />
										<div>.</div>
...
jQuery(document).ready(function(){
	//startTimer("distributionToFieldStaff");
		...
		if(category.trim()!=''){
			  if(tenant=="lalteer"){
				  $.ajax({
						 type: "POST",
				        async: false,
				        url: "distributionToFieldStaff_loadAvailableStockLalteer.action",
			...
			$.ajax({
				 type: "POST",
		        async: false,
		        url: "distributionToFieldStaff_loadAvailableStock.action",
		        data: {selectedWarehouse:selectedWarehouse,selectedProduct:selectedProduct,selectedUnit:selectedUnit,selectedSeason:selectedSeason,batchNo:batchNo},
		        success: function(data) {
			if(data!=null && data!=""){
				if(data.indexOf("j_username")!= -1){
					redirectForm();
...
function addRow(){
	...
	if(isPartial){
		...
		QtyPerUnit=(parseFloat(distributionStock)).toFixed(3);
	...
	if(editIndex==-1){
	 ...
	 productArray = {
			 ...
			 subUnit:subUnit,
			 enteredStock:parseFloat(distributionStock).toFixed(3),
	 ...
	 for(var i=0;i<productsInfoArray.length;i++){
		...
		productsInfoArray[editIndex].enteredStock=parseFloat(distributionStock).toFixed(3);
...
function reloadTable(){
	...
	if(rowCount==0){
		tbodyRow += '<tr>'+
					'<td colspan="8" style="text-align:center"><s:text name="noRecordFoundDist"/></td>'+
					'</tr>';
	...
	if(enableBatchNo=='1'){
		tfootRow += '<tr>'+
		'<td style="border:none!important;">&nbsp;</td>'+
	'<td colspan="5" align="right" style="border:none!important; padding-right:10px!important; padding-top:6px!important; padding-bottom:6px!important;"><s:text name="total"/></td>';
}else{
	tfootRow += '<tr>'+
	'<td style="border:none!important;">&nbsp;</td>'+
  ...
  if(selectedSeason=="" &&  selectedSeason==0){
	  ...
	  //$("#validateError").innerHTML='<s:text name="emptyCooperative"/>';
		...
		return false;
  		...
  		// $("#validateError").innerHTML='<s:text name="noRecordFoundDist"/>';
		hit=false;
		enableButton();
		return false;
	}
	...
	$.post("distributionToFieldStaff_populateDistribution.action",{selectedSeason:selectedSeason,productTotalString:productTotalString,selectedWarehouse:selectedWarehouse,selectedAgent:selectedAgent,startDate:selectedDate,costPriceArrayValue:costPriceString,selectedProduct:selectedProduct},function(data){
		if(data == null || data == ""){		
...
function disablePopupAlert(){
	document.redirectform.action="distributionToFieldStaff_list.action";
	document.redirectform.submit();
}

function distributionEnable(){
	document.redirectform.action="distributionToFieldStaff_create.action";
	document.redirectform.submit();
}
...
var category=$("#category").val();
		if(category.trim()!=''){
			...
			if(tenant=="lalteer"){
				$.post("distributionToFieldStaff_loadAvailableStockLalteer",{selectedProduct:selectedProduct,selectedWarehouse:selectedWarehouse,selectedUnit:selectedUnit,batchNo:batchNo},function(data){
					var n=data.split(",");
					for(var i=0; i< productsInfoArray.length; i++){
	...
	$.post("distributionToFieldStaff_loadAvailableStock",{selectedProduct:selectedProduct,selectedWarehouse:selectedWarehouse,selectedUnit:selectedUnit,selectedSeason:selectedSeason,batchNo:batchNo},function(data){
		var n=data.split(",");
		for(var i=0; i< productsInfoArray.length; i++){
			if( !productsInfoArray[i].isEdit && productsInfoArray[i].name===selectedProduct.split('=')[0] && productsInfoArray[i].units===selectedUnit && productsInfoArray[i].batchNo===batchNo){
				n[0]= parseFloat(parseFloat(n[0]).toFixed(3))-parseFloat(parseFloat(productsInfoArray[i].QtyPerUnit).toFixed(4));
...
function listAgent(call){
	$.post("distributionToFieldStaff_populateFieldStaff", {selectedWarehouse: call.value}, function (data) {
        var result = data;
...
/*function populateProduct(call){	
    ...
    success: function (oResponse) 	{   
		...
		addOption(document.getElementById("product1"), "Select", "");
				for (var i=0; i < arry.length;i++)
			if(arry[i]!="")
		addOption(document.getElementById("product1"), arry[i], arry[i]);
				
		}
	
	}
    var data = "selectedWarehouse="+call.value;	 
	var url='distributionToFieldStaff_populateProduct';	   
 ...
 function populateGroup(){
			...
			var availableGroup;
			$.post("distributionToFieldStaff_populateGroup",{selectedAgent:selectedAgent},function(data){
					if(data!=null && data!=""){
...
//alert(lable);
		...
		var availableGroup;
		$.post("distributionToFieldStaff_populateUnit",{selectedProduct:selectedProduct},function(data){
				if(data!=null && data!=""){
					document.getElementById("productUnit").innerHTML=data;
					/* document.getElementById("pricePerUnit").innerHTML= "Price / "+data;	
					document.getElementById("distributionStock").innerHTML= distributionStockLable+"("+data+")"; */	
					//document.getElementById("distributionStockUnitLable").innerHTML= document.getElementById("distributionStockLable").innerHTML+"("+data+")";	
				}
				else{
...
//alert(lable);
		...
		//alert(selectedCategory);
		var availableGroup;
		$.post("distributionToFieldStaff_populateUnitLalteer",{selectedCategory:selectedCategory},function(data){
			if(data!=null && data!=""){
				document.getElementById("productUnit").innerHTML=data;
			}
			else{
   ...
   function populateProduct(call){	
	    ...
	    $.post("distributionToFieldStaff_populateProduct", {selectedWarehouse: call.value}, function (result) {
...
function populateSubCategory()
{
	...
	if(seasonCode!='' && selectedWarehouse!='')
      ...
      $.post("distributionToFieldStaff_populateSubCategory", {selectedWarehouse:selectedWarehouse,seasonCode:seasonCode}, function (result) {
...
function insertOptions(ctrlName, jsonArr) {
    ...
    addOption(document.getElementById(ctrlName), "Select", "-1");
...
function populateMobileUser(){
	var selectedWarehouse = document.getElementById("warehouse").value;
	$.post("distributionToFieldStaff_populateMobileUser", {selectedWarehouse:selectedWarehouse}, function (result) {
     	 insertOptions("agentId",JSON.parse(result));
     });
}
function populateLalteerSubCategory(){
	...
	if(selectedWarehouse!='')
      ...
      $.post("distributionToFieldStaff_populateLalteerSubCategory", {selectedWarehouse:selectedWarehouse}, function (result) {
      	 insertOptions("category",JSON.parse(result));
      });
...
function listBatchNo()
{
	...
	var product=document.getElementById("product1").value;
		...
		$.ajax({
			 type: "POST",
	        async: false,
	        url: "distributionToFieldStaff_populateBatchNo.action",
...
function listProduct()
{
	...
	if(selectedWarehouse.trim()!='' && category.trim()!=''){
		...
		$.ajax({
			 type: "POST",
	        async: false,
	        url: "distributionToFieldStaff_populateProductBasedOnCategory.action",
...
function listProductLalteer()
{
	...
	if(selectedWarehouse.trim()!='' && category.trim()!=''){
		...
		$.ajax({
			 type: "POST",
	        async: false,
	        url: "distributionToFieldStaff_populateProductBasedOnCategoryLalteer.action",
	        data: {selectedCategory:category,selectedWarehouse:selectedWarehouse},
	        success: function(result) {
	        	 insertOptions("product1",JSON.parse(result));
	        }
   ...
   /*function disablePopupAlert(){
		...
		$('#restartAlert').hide();
		$('body').css('overflow','');
		document.form.action = "distributionToFieldStaff_create.action";
...
<s:form name="redirectform"
	action="distributionToFieldStaff_list.action" method="POST">
