...
<div class="filterEle flexi flexi12 farmCropClass">
					...
					<div class="form-element">
						<s:select name="crop" id="cropId" list="cropList" headerKey=""
							headerValue="%{getText('txt.select')}"
							class="form-control crop cond input-sm select2" />
					</div>
				</div>
	...
	<sec:authorize ifAllGranted="service.dynamicCertification.create">
		<input type="BUTTON" id="add" class="btn btn-sts"
			...
			onclick="createFormSub()" />
	</sec:authorize>
	<s:form name="createform" action="dynamicCertification_create">
	...
	$('.filterss').empty();
		...
		function loadGrid(){
			...
			jQuery("#detail").jqGrid({
				url:'dynamicCertification_data.action?',
			 	...
			 	onSelectRow: function(id){ 
					var act =  "dynamicCertification_detail.action?txnType="+typez+"&id="+id
...
function gerRefId(){
		...
		window.location.href="dynamicCertification_list?txnType="+typez;
	//	window.location.href="dynamicCertification_list?txnType="+typez;
		
		}
		function createFormSub(){
			document.createform.action=	"dynamicCertification_create?txnType="+$('#txnType').val();
		...
		function exportXLS(objId) {
			...
			jQuery("#detail").jqGrid('excelExport', {
				url : 'dynamicCertification_populateXLS.action?id='+objId,
				postData : dataa
			});
		}
		...
		function reloadGrid() {
			...
			//jQuery("#detail").jqGrid('setGridParam',{url:"dynamicCertification_data.action?",page:1}).trigger('reloadGrid');
