...
<s:form name="form" cssClass="fillform"
	action="productReturnFromFarmer_%{command}">
			...
			<div class="service-content-section">
				...
				<div class="appContentWrapper marginBottom">
					<div class="formContainerWrapper">
						...
						<div class="flexiWrapper filterControls">
							...
							<div class="flexi flexi10" style="width:200px">
								...
								<div class="form-element">
									...
									<s:else>
										<s:select name="selectedProduct"
											...
											list="productsNameUnitMap" headerKey=""
											headerValue="%{getText('txt.select')}" id="product1"
											onchange="resetUnitData();loadUnit();" />
						...
						<%-- 	<div class="flexi flexi10">
							...
							<%-- <div class="flexi flexi10">
								<label for="txt" style="display: block; text-align: center"><s:text
										name="distribution.sellingPrice" />(<s:property value="%{getCurrencyType().toUpperCase()}" />)</label>
								<div class="form-element">
									<span id="costPrice" style="display: block; text-align: center" />
								</div>
							</div> --%>
							<%-- <div class="flexi flexi10" id="sellingRupeeTd">
								<label for="txt"><s:text
							...
							<%-- <div class="flexi flexi10" id="totalPriceDiv">
								<label for="txt" style="display: block; text-align: center"><s:text
										name="totalPrice" /></label>
								<div class="form-element">
				...
				<th width="11%"><s:text name="totalQuantity" /></th>  -->
										...
										<%-- <th width="8%" id="costPriceLable" style="text-align: center"><s:text
												name="distribution.sellingPrice" />(<s:property value="%{getCurrencyType().toUpperCase()}" />)</th> --%>

										<th width="9%" style="text-align: center"><s:text
												name="quantity" /></th>
									<%-- 	<th width="11%" id="finalPriceLabel">
										<s:text
												name="totalPrice" />
									<s:property
								value="%{getLocaleProperty('totalPrice')}" />(<s:property value="%{getCurrencyType().toUpperCase()}" />)
								</th> --%>
								...
								<tbody>
									<tr>
										<td colspan="5" style="text-align: center;"><s:text
												name=" " /></td>
									</tr>
								</tbody>
								<tfoot id="footerId">
...
<s:if test="prodReturnImgAvil==1">		
					<s:form name="prodReturnImageform" cssClass="fillform" action="productReturnFromFarmer_createImage.action"
				method="post" enctype="multipart/form-data" id="imgForm">
					<s:hidden key="currentPage" />
					<s:hidden key="id" id="distId" name="distId" />
				<div class="appContentWrapper marginBottom" id="prodReturnDiv"
					style="display: block;">
					<div class="formContainerWrapper">
						<h2>
						...
						<div class="flexiWrapper filterControls">
							...
							<div class="flexform-item">
								<label for="txt"> <s:property
								value="%{getLocaleProperty('prodReturn.image2')}" /> <span
									style="font-size: 8px"> <s:text name="farmer.imageTypes" />
								<font color="red"> <s:text name="imgSizeMsg" /></font>
								</span>
								</label>
								<div class="form-element">
								<s:file name="prodReturnImg2" id="prodReturnImg2"
									onchange="checkImgHeightAndWidth(this)" cssClass="form-control" />
								</div>
							</div>
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">
					<s:text name="productreturn.success" />
				</h4>
			</div>
...
var prodReturnImageEnable='<s:property value="%{getProdReturnImgAvil()}" />';
...
function listFarmer(call){
	...
	if( call.value==""){
	 ...
	 $.post("productReturnFromFarmer_populateFarmer", {selectedVillage: call.value,selectedWarehouse:selectedWarehose}, function (data) {
				/*var result=data;
...
function listCostPrice(){
	...
	if(tenant!="lalteer"){
	$.post("productReturnFromFarmer_populateCostPrice",{selectedCooperative:selectedCooperative,selectedProduct:selectedProduct,selectedAgent:selectedAgent},function(data){
			if(data!=null && data!=""){
				//alert("aaa");
	...
	$.post("productReturnFromFarmer_populateLalteerCostPrice",{selectedCooperative:selectedCooperative,selectedProduct:selectedProduct,selectedAgent:selectedAgent},function(data){
		if(data!=null && data!=""){
			//alert("aaa");
			document.getElementById("costPrice").innerHTML=data;
			document.getElementById("selllingRupee").value=data;
...
function listProductLalteer(call){	
	...
	$.ajax({
		 type: "POST",
         async: false,
         url: "productReturnFromFarmer_populateLalteerProduct",
         ...
         success: function(result) {
		  ...
		  /* $.post("productReturnFromFarmer_populateProduct", {selectedCategory: call.value,selectedCooperative:selectedCooperative}, function (result) {
			  insertOptions("product1",JSON.parse(result));
      }); */
...
function listProduct(call){	
	...
	$.ajax({
		 type: "POST",
         async: false,
         url: "productReturnFromFarmer_populateProduct",
         data: {selectedCategory: call.value},
         success: function(result) {
        	 if(result.length==0){
...
function populateProductCooperative(call){	
	  ...
	  $.post("productReturnFromFarmer_populateProductCooperative", {selectedCooperative: call.value}, function (result) {
...
function populateWarehouseCategory(){	
	...
	$.post("productReturnFromFarmer_populateWarehouseCategory", {selectedCooperative:selectedCooperative,seasonCode:seasonCode}, function (result) {
...
if(selectedCooperative.length==0){
	...
	$.post("productReturnFromFarmer_populateLalteerWarehouseCategory", {selectedCooperative:selectedCooperative}, function (result) {
   	insertOptions("category",JSON.parse(result));
   });
...
function populateAgentCategory(call){	
	 var selectedSeason= document.getElementById("season").value;
	  ...
	  $.post("productReturnFromFarmer_populateAgentCategory", {selectedAgent: call.value,selectedSeason:selectedSeason}, function (result) {
...
function populatelalteerAgentCategory(call){	
	 var selectedSeason= document.getElementById("season").value;
	  ...
	  $.post("productReturnFromFarmer_populateLalteerAgentCategory", {selectedAgent: call.value,selectedSeason:selectedSeason}, function (result) {
	 ...
	 var selectedSeason= document.getElementById("season").value;
		...
		if(selectedAgent!=null && selectedAgent!="")
		{
			$.ajax({
				 type: "POST",
		         async: false,
		         url: "productReturnFromFarmer_populateProductFieldStaff",
	...
	var selectedAgent = document.getElementById("fieldstaff").value;
		...
		if(selectedAgent!=null && selectedAgent!="")
		{
			$.ajax({
				 type: "POST",
		         async: false,
		         url: "productReturnFromFarmer_populateLalteerProductFieldStaff",
...
function listVillage(call){
	...
	if(selectedValue==""){
		...
		addOption(document.form.farmer, '<s:text name="txt.select"/>', "");
	     ...
	     $('#village').html('');
	     addOption(document.getElementById("village"), '<s:text name="txt.select"/>', "");
		 $.post("productReturnFromFarmer_populateVillage", {selectedWarehouse: selectedValue}, function (data) {
		var result=data;
...
function listVillageForUnregisteredFarmer(){
	 ...
	 $.post("productReturnFromFarmer_populateVillageForUnRegisteredFarmer", function (result) {		
			document.form.village.length = 0;
			addOption(document.getElementById("village"),'<s:text name="txt.select"/>', "");
...
function validateDetails(validateEmpty){
  ...
  // var sellRupee=$('#selllingRupee').val();
	...
	/* else if((validateEmpty  || distributionStock.trim()!="")){
		alert(distributionStock.trim()+"3");
			document.getElementById("validateError").innerHTML='<s:text name="insufficientstock"/>';
			hit=false;
	...
	else if((validateEmpty  || distStock.trim()!="")  ){
		alert("5");
			document.getElementById("validateError").innerHTML='<s:text name="insufficientstock"/>';
			hit=false;
	}
...
if(hit) {
	...
	document.getElementById("distributionStock").disabled=false;
		...
		$.ajax({
			 type: "POST",
	        async: false,
	        url: "productReturnFromFarmer_populateBatchNo.action",
...
function availableStock(){
	...
	$.ajax({
			 type: "POST",
	         async: false,
	         url: "productReturnFromFarmer_loadAvailableStock",
	         data: {selectedFarmer:selectedFarmer,selectedProduct:selectedProduct,selectedUnit:selectedUnit,selectedAgent:selectedAgent,selectedCategory:selectedCategory,selectedSeason:selectedSeason,batchNo:batchNo,selectedCooperative:selectedCooperative},
			...
			$.ajax({
					 type: "POST",
			         async: false,
			         url: "productReturnFromFarmer_loadAvailableStockLalteer",
...
function loadUnit(){
	...
	var availableUnit;
	$.post("productReturnFromFarmer_populatePopulateProductUnit",{selectedProduct:selectedProduct},function(data){
...
function addRow(){
	...
	var totalStockPrice=calculateTotalPrice(distributionStock,isPartial,units.split('-'),subUnit);	
	//console.log(coPrice);
	...
	if(editIndex==-1){
	 ...
	 for(var i=0;i<productsInfoArray.length;i++){
		 if(productsInfoArray[i].productId== productArray.productId && productsInfoArray[i].batchNo== productArray.batchNo){
				...
				//productsInfoArray[i].coPrice = coPrice;
...
function reloadTable(){
	...
	if(rowCount==0){
		tbodyRow += '<tr>'+
					'<td colspan="11" style="text-align:center"><s:text name="noRecordFoundProdReturnFromFarmer"/></td>'+
					'</tr>';
	}
		
	jQuery('#productInfoTbl > tbody').html(tbodyRow);
	var taxAmntValue = 0.0;
	var tfootRow = "";
...
/* 	tfootRow += '<tr>'+
	...
	tfootRow+='<td class="alignRight"><s:textfield name="tax" id="amntTax" onkeyup="calAmntWithTax();" onkeypress="return isDecimal(event)"  cssStyle="text-align:center;padding-right:1px; width:140px!important;"/></td>';
			  '</tr>'; */
    tfootRow += '<tr>'+
...
function submitProductReturn() {
    ...
    if(selectedSeason=="" &&  selectedSeason==0){
		...
		document.getElementById("validateError").innerHTML='<s:property value="%{getLocaleProperty('invalid.farmerName')}" />';
		enableButton();
		hit=false;
		return false;
	}
		...
		document.getElementById("validateError").innerHTML='<s:text name="noRecordFoundProdReturnFromFarmer"/>';
		enableButton();
		hit=false;
		return false;
	}
...
/* 	if(!isFree){
	...
	if(hit){
		var finalAmtValue = document.getElementById("totalAmtVal").innerHTML;
	...
	//	productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+0+"##"+0+"##"+productTotalArray[i].coPrice+"##"+productTotalArray[i].batchNo+"||";
				productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+0+"##"+0+"##"+productTotalArray[i].batchNo+"||";
		  }else{
			//  productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+0+"##"+0+"##"+productTotalArray[i].coPrice+"##"+'NA'+"||";
			  productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+0+"##"+0+"##"+'NA'+"||";
		  }
	  }else{
		  if($("#enableBatchNo").val()=='1'){
			//productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+productTotalArray[i].coPrice+"##"+productTotalArray[i].totalStockPrice+"##"+productTotalArray[i].coPrice+"##"+productTotalArray[i].batchNo+"||";
			  productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+productTotalArray[i].totalStockPrice+"##"+productTotalArray[i].batchNo+"||";
		  }else{
			 //productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+productTotalArray[i].coPrice+"##"+productTotalArray[i].totalStockPrice+"##"+productTotalArray[i].coPrice+"##"+'NA'+"||";
			  productTotalString+=productTotalArray[i].name+"##"+productTotalArray[i].qty+"##"+productTotalArray[i].totalStockPrice+"##"+'NA'+"||";
		  }	  
	}
	}
	var farmerIdOrName;
	...
	$.post("productReturnFromFarmer_populateProductReturn.action",{selectedSeason:selectedSeason,selectedCooperative:selectedWarehouse,selectedWarehouse:selectedSamithi,selectedVillage:selectedVillage,selectedDate:selectedDate,productStock:stockType,farmerId:farmerIdOrName,
		registeredFarmer:farmerType,productTotalString:productTotalString,selectedAgent:selectedAgent
		,paymentRupee:paymentRupee,paymentPaise:paymentPaise,unRegisterFarmerName:unRegisterFarmerName,taxValue:taxValue,finalAmtValue:finalAmtValue,paymentMode:paymentMode,selectedFarmerValue:selectedFarmerValue,freeDistribution:isFreeDistribution,mobileNo:mobileNo},function(data){
			//alert("data:"+JSON.stringify(data.id));
			if(data == null || data == ""){				
			}
	else{ 	 			
		if(prodReturnImageEnable=="1"){
 			$("#distId").val(data.id);
 			prodReturnImageUpload();	
		}
...
function prodReturnImageUpload(){
	$("#imgForm").submit();
}

function validateImage(){
	
	var file1=document.getElementById('prodReturnImg1').files[0];
	var filename1=document.getElementById('prodReturnImg1').value;
	var fileExt1=filename1.split('.').pop();
	var file2=document.getElementById('prodReturnImg2').files[0];
	var filename2=document.getElementById('prodReturnImg2').value;
...
function disablePopupAlert(){
	...
	$('#restartAlert').hide();
	$('body').css('overflow',''); */
	document.redirectform.action="productReturnFromFarmer_list.action";
...
function calculateTotalPrice(distributionStock,isPartial,units,subUnit){
	...
	if(!isPartial)
	{
		...
		if(units[1].toUpperCase()=="KG" && subUnit.toUpperCase()=="GM"){
			...
			return (parseFloat(toatalQty)*parseFloat(distributionStock)).toFixed(2);
	}
...
/* 	var sellRupee=$('#selllingRupee').val();
	...
	$("#stock").text('');
	if(tenant=="lalteer"){
	$.post("productReturnFromFarmer_loadAvailableStockLalteer",{selectedProduct:selectedProduct,selectedCooperative:selectedCooperative,selectedUnit:selectedUnit,selectedAgent:selectedAgent,selectedCategory:selectedCategory,batchNo:batchNo},function(data){
		var n=data.split(",");
		for(var i=0; i< productsInfoArray.length; i++){
	   ...
	   //jQuery("#totalPriceLabel").html('');
		...
		$.post("productReturnFromFarmer_loadAvailableStock",{selectedProduct:selectedProduct,selectedCooperative:selectedCooperative,selectedUnit:selectedUnit,selectedAgent:selectedAgent,selectedCategory:selectedCategory,selectedSeason:selectedSeason,batchNo:batchNo},function(data){
			var n=data.split(",");
			for(var i=0; i< productsInfoArray.length; i++){
...
function editRow(indx){
	...
	var urlEdit;
	if(tenant=="lalteer"){
		urlEdit="productReturnFromFarmer_populateLalteerProduct";
	}else{
		urlEdit="productReturnFromFarmer_populateProduct";
	}
...
/* function loadFarmerAccountBalance()
{
	...
	$.post("productReturnFromFarmer_populateFarmerAccBalance",{selectedFarmerValue:selectedFarmerValue},function(data){
...
function listVillageByCity(obj){
	...
	jQuery.post("productReturnFromFarmer_populateVillageByCity.action",{id:obj.value,dt:new Date(),selectedCity:obj.value},function(result){
...
var distributionStockLable="<s:text name='distributionStock'/>";
	...
	$.post("productReturnFromFarmer_populateUnit",{selectedCategory:selectedCategory},function(data){
			if(data!=null && data!=""){				
...
<s:form action="productReturnFromFarmer_populatePrintHTML"
...
<s:form name="redirectform" action="productReturnFromFarmer_list.action"
	method="POST">
</s:form>