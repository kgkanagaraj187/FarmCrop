...
function loadInspectionCheckGrid(){
	jQuery("#inspectionCheckDetail").jqGrid({
		 url:'inspectionCheck_data.action?id='+ document.getElementById('farmerId').value,
       pager: '#inspectionCheckPagerForDetail',
       mtype: 'POST',
       datatype: "json",
       mtype: 'POST',			
...
{name:'action',index:'action',width:125,sortable:false,search:false}</sec:authorize>--%>
      				...
      				act = act.split("?txnType=")[0]+"?txnType=387&id="+id;
      			}
      			...
      			colModel = jQuery("#inspectionCheckDetail").jqGrid('getGridParam', 'colModel');
      		    $('#gbox_' + $.jgrid.jqID(jQuery("#inspectionCheckDetail")[0].id) +
      		    ...
      		    $('#inspectionCheckDetail').jqGrid('setGridHeight',(reportWindowHeight));
                   }
...
<div style="width: 99%;" id="baseDiv">
	<table id="inspectionCheckDetail"></table>
	<div id="inspectionCheckPagerForDetail"></div>
</div>

<s:form name="updateform" id="updateform"
	action="inspectionCheck_detail.action">
	<%-- 	<s:hidden key="id" /> --%>
