...
function wrapToDiv(component, val) {
	...
	if (val.compoType == '12') {
	label.append($("<label style='color: red;width:100%;font-size:10px'/>").html("Image size should be less than 1 MB"));
	}	
...
function wrapToList(component, val) {
	...
	/*if (!isEmpty(val.isReq) && val.isReq == '1') {
		// tr.append($("<sup style='color: red;'/>").html("*"));
		wrappedDiv.append($("<h1 style='color: red;'/h6>").html("*"));
								...
								.each(
										function(key, val) {
											...
											/*if (val.isActPlan !=null) {
												...
												$(".acPlanDet" + val.code)
												.append(
														"<button type='button' class='btn btn-sm pull-right photo imgCenter photoModel'><i class='fa fa-picture-o' aria-hidden='true'></i></button>");
											}*/
											if (val.photoCompoAvailable == "1") {
												var ids = val.photoIds;
												$(".imgTd" + val.code)
	...
	$('#jquery_jplayer_1').jPlayer({
		ready: function () {
			$(this).jPlayer("setMedia", {
				m4v: "farmer_populateVideoPlay?imgId="+id+"&dt="+new Date()
			}).jPlayer("play");
		},
