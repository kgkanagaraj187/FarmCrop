/*
 * $Id: Table.java 389 2013-02-18 07:57:19Z pavel-alay $
 *
 ...
 *
 */
public class TableTagProcessor extends AbstractTagProcessor {
	public static final float DEFAULT_CELL_BORDER_WIDTH = 0.75f;

	private static final Logger LOG = LoggerFactory.getLogger(TableTagProcessor.class);
	...
	public List<Element> end(final WorkerContext ctx, final Tag tag, final List<Element> currentContent) {
		try {
			...
			// float targetWidth = calculateTargetWidth(tag, columnWidths,
			// outerWidth, ctx);
			float targetWidth = 0;
			...
			} else if (null == tag.getParent() || (null != tag.getParent()
					&& htmlPipelineContext.getRootTags().contains(tag.getParent().getName()))) {
				targetWidth = max;
			} else /*
					 * this table is an inner table and width adjustment is done
					 * in outer table
					 */ {
			...
			if (totalFixedColumnWidth == initialTotalWidth) { // all column
																// widths are
																// fixed
				...
				if (initialTotalWidth > targetWidth) {
					...
					if (leftToReduce != 0) {
						...
						} else { // set all none fixed columns to their minimum,
									// with the
							// widestWord array.
			...
			Float tableHeight = new HeightCalculator().getHeight(tag,
					getHtmlPipelineContext(ctx).getPageSize().getHeight());
			Float tableRowHeight = null;
			if (tableHeight != null && tableHeight > 0)
				tableRowHeight = tableHeight / tableRows.size();
			int rowNumber = 0;
			for (TableRowElement row : tableRows) {
				int columnNumber = -1;
				Float computedRowHeight = null;
				/*
				 * if (tableHeight != null && tableRows.indexOf(row) ==
				 * tableRows.size() - 1) { float computedTableHeigt =
				 * table.calculateHeights(); computedRowHeight = tableHeight -
				 * computedTableHeigt; }
				 */
				for (HtmlCell cell : row.getContent()) {
						...
						for (Element baseLevel : compositeElements) {
							if (baseLevel instanceof PdfPTable) {
								...
								float totalBordersWidth = cellValues.isLastInRow()
										? styleValues.getHorBorderSpacing() * 2 : styleValues.getHorBorderSpacing();
								totalBordersWidth += cellValues.getBorderWidthLeft() + cellValues.getBorderWidthRight();
								float columnWidth = 0;
								for (int currentColumnNumber = columnNumber + 1; currentColumnNumber <= columnNumber
										+ cell.getColspan(); currentColumnNumber++) {
	 ...
	 * <ol>
	 * <li>if the attribute or style "width" is found in the given tag and it is
	 * not wider than pageWidth - outerWidth, then the targetWidth = width
	 * value</li>
	 * <li>if the columnWidths array in total is not wider than pageWidth -
	 * outerWidth, then the targetWidth = the total of the columnWidths
	 * array</li>
	 * <li>if table's parent is a root tag or table has no parent, then the
	 * targetWidth = width of the page - outerWidth
	 * {@link TableTagProcessor#getTableOuterWidth(Tag, float, WorkerContext)}.</li>
	 * </ol>
	 * If none of the above is true, the width of the table is set to its
	 * default with the columnWidths array.
	 *
	 * @param tag
	 *            containing attributes and css.
	 * @param columnWidths
	 *            float[] containing the widest lines of text found in the
	 *            columns.
	 * @param outerWidth
	 *            width needed for margins and borders.
	 * @param ctx
	 ...
	 *
	 * @param tableRows
	 *            List of {@link TableRowElement} objects of the table.
	...
	public static TableStyleValues setStyleValues(final Tag tag) {
		...
		if (attributes.containsKey(CSS.Property.BORDER)) {
			...
			for (Entry<String, String> entry : css.entrySet()) {
				...
				} else if (key.equalsIgnoreCase(CSS.Property.BORDER_RIGHT_STYLE)
						&& CSS.Value.SOLID.equalsIgnoreCase(value)) {
				...
				} else if (key.equalsIgnoreCase(CSS.Property.BORDER_BOTTOM_STYLE)
						&& CSS.Value.SOLID.equalsIgnoreCase(value)) {
	...
	public static TableStyleValues setBorderAttributeForCell(final Tag tag) {
		...
		if (border != null && (border.trim().length() == 0
				|| utils.parsePxInCmMmPcToPt(attributes.get(CSS.Property.BORDER)) > 0)) {
			styleValues.setBorderColor(BaseColor.BLACK);
			styleValues.setBorderWidth(TableTagProcessor.DEFAULT_CELL_BORDER_WIDTH);
		}
	 ...
	 *
	 * @param getHor
	 *            true for horizontal spacing, false for vertical spacing.
	 * @param css
	 *            of the table tag.
	 * @param attributes
	 *            of the table tag.
	 ...
	 * <ul>
	 * <li>cell width = {@link TableTagProcessor#getCellStartWidth(HtmlCell)} +
	 * the width of the widest line of text.</li>
	 * <li>widest word = {@link TableTagProcessor#getCellStartWidth(HtmlCell)} +
	 * the widest word of the cell.</li>
	 * </ul>
	 * These 2 widths are used as the starting point when determining the width
	 * of the table in
	 *
	 * @param cell
	 *            HtmlCell of which the widths are needed.
				...
				if (baseLevel instanceof Phrase) {
					for (int i = 0; i < ((Phrase) baseLevel).size(); i++) {
						...
						if (inner instanceof Chunk) {
							...
							float widestWord = startWidth
									+ getCssAppliers().getChunkCssAplier().getWidestWord((Chunk) inner);
					...
					for (PdfPRow innerRow : ((PdfPTable) baseLevel).getRows()) {
						...
						for (PdfPCell innerCell : innerRow.getCells()) {
							celnr++;
							if (innerCell != null) {
								float innerWidestWordOfCell = setCellWidthAndWidestWord(
										new HtmlCell(innerCell, celnr == size))[1];
	 ...
	 *
	 * @param widths
	 *            array of floats containing column width values.
	 * @param outerWidth
	 *            equals the required space outside of the table for margins and
	 *            borders.
	...
	private float getTableOuterWidth(final Tag tag, final float horBorderSpacing, final WorkerContext ctx)
		...
		float total = utils.getLeftAndRightMargin(tag, getHtmlPipelineContext(ctx).getPageSize().getWidth())
				...
				+ utils.checkMetricStyle(tag, CSS.Property.BORDER_RIGHT_WIDTH) + horBorderSpacing;
	 ...
	 *
	 * @param table
	 *            PdfPTable on which the margins need to be set.
	 * @param t
	 *            Tag containing the margin styles and font size if needed.
	 * @param values
	 *            {@link TableStyleValues} containing border widths and border
	 *            spacing values.
	 * @param ctx
	 * @throws NoCustomContextException
	 */
