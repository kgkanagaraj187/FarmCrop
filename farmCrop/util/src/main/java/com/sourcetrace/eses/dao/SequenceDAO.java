...
import com.ese.entity.util.CustomerProjectSequence;
...
public class SequenceDAO extends ESEDAO implements ISequenceDAO {
    ...
    /*
     * (non-Javadoc)
     * @see com.ese.dao.profile.ISequenceDAO#getShopDealerRemoteIdSeq()
     */
    public synchronized String getShopDealerRemoteIdSeq() {
        ...
        query = session.createSQLQuery("SELECT REMOTE_SEQ FROM " + SHOP_DEALER_ID_SEQ);
        selectedSeq = Integer.parseInt((String) query.list().get(0));
        updatedSeq = selectedSeq + ShopDealer.SHOP_DEALER_SEQ_RANGE;
        if (updatedSeq >= ShopDealer.SHOP_DEALER_ID_MAX_RANGE) {
            updatedSeq = ShopDealer.SHOP_DEALER_ID_MAX_RANGE;
        }
        query = session
                .createSQLQuery("UPDATE " + SHOP_DEALER_ID_SEQ + " SET REMOTE_SEQ = " + updatedSeq);
        int i = query.executeUpdate();

        session.flush();
        session.close();
        String remoteShopDealerId = StringUtil.appendZeroPrefix(String.valueOf(selectedSeq),
                ShopDealer.SHOP_DEALER_ID_LENGTH);
        return String.valueOf(remoteShopDealerId);
    }

    /*
     * (non-Javadoc)
     * @see com.ese.dao.profile.ISequenceDAO#getShopDealerWebIdSeq()
     */
    @SuppressWarnings("unchecked")
    public synchronized String getShopDealerWebIdSeq() {

        int selectedSeq = 0;
        int selectedSeqLimit = 0;
        int updatedSeq = 0;
        Session session = getSessionFactory().openSession();
        SQLQuery query = null;

        query = session.createSQLQuery("SELECT WEB_SEQ, WEB_SEQ_LIMIT FROM " + SHOP_DEALER_ID_SEQ);
        List<Object[]> objectList = query.list();
        selectedSeq = Integer.parseInt((String) objectList.get(0)[0]);
        selectedSeqLimit = Integer.parseInt((String) (String) objectList.get(0)[1]);
        updatedSeq = selectedSeq + 1;

        if (updatedSeq <= selectedSeqLimit) {
            query = session.createSQLQuery(
                    "UPDATE " + SHOP_DEALER_ID_SEQ + " SET WEB_SEQ = " + updatedSeq);
            int i = query.executeUpdate();
        } else {
            String newShopDealerSeqId = getShopDealerRemoteIdSeq();
            int newShopDealerId = Integer.parseInt(newShopDealerSeqId);
            updatedSeq = newShopDealerId + 1;
            int newShopDealerIdLimit = newShopDealerId + ShopDealer.SHOP_DEALER_SEQ_RANGE;
            if (newShopDealerIdLimit >= ShopDealer.SHOP_DEALER_ID_MAX_RANGE) {
                newShopDealerIdLimit = ShopDealer.SHOP_DEALER_ID_MAX_RANGE;
            }
            query = session.createSQLQuery("UPDATE " + SHOP_DEALER_ID_SEQ + " SET WEB_SEQ = "
                    + updatedSeq + ", WEB_SEQ_LIMIT = " + newShopDealerIdLimit);
            int i = query.executeUpdate();
        }
        session.flush();
        session.close();

        String webShopDealerId = StringUtil.appendZeroPrefix(String.valueOf(updatedSeq),
                ShopDealer.SHOP_DEALER_ID_LENGTH);
        return webShopDealerId;
    }
    ...
    public Long getProcurementVarietyIdSeq() {
        ...
        // TODO Auto-generated method stub
        return getESESequence(VARIETY_ID_SEQ);
    }
    ...
    public String getGroupHHIdSeq(String code) {
        ...
        Session session = getSessionFactory().openSession();
        Query query = session.createQuery("SELECT MAX(code) FROM Warehouse WHERE code LIKE :code");
	...
	public Long getFarmCropsMasterCodeSeq() {
		  return getESESequence(FARM_CROPS_MASTER_CODE_SEQ);
		 }
