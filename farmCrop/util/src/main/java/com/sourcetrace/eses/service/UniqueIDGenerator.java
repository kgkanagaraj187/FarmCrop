...
//	protected HttpServletRequest request;
	 ...
	 */
	public String getShopDealerRemoteIdSeq() {

		return sequenceDAO.getShopDealerRemoteIdSeq();
	}
	 ...
	 */
	public String getShopDealerWebIdSeq() {

		return sequenceDAO.getShopDealerWebIdSeq();
	}
	 ...
	 * .lang.String)
	 */
	public String createCustomerProjectCode(String customerId) {

		String startingSeq = "0";
		String finalSequence = null;

		try {

			CustomerProjectSequence customerProjectSeq = sequenceDAO.findProjectCodeSequenceByCustomerId(customerId);
			if (!ObjectUtil.isEmpty(customerProjectSeq)) {

				if (!StringUtil.isEmpty(customerProjectSeq.getProjectCodeSeq())) {

					Integer updatedId = Integer.parseInt(customerProjectSeq.getProjectCodeSeq()) + 1;

					finalSequence = customerId
							+ StringUtil.getExact(String.valueOf(updatedId), CUSTOMER_PROJECT_CODE_LENGTH);

				} else {

					Integer updatedId = Integer.parseInt(startingSeq) + 1;
					finalSequence = customerId
							+ StringUtil.getExact(String.valueOf(updatedId), CUSTOMER_PROJECT_CODE_LENGTH);
				}
			} else {
			...
			LOGGER.info("Error at Customer Project Code creation time");
			e.printStackTrace();
		}
	 ...
	 * 
	 * @see
	 * com.ese.service.util.IUniqueIDGenerator#saveCustomerProjectSequence(java
	 * .lang.String, java.lang.String)
	 */
	public void saveCustomerProjectSequence(String customerId, String customerProjectCodeSeq) {

		CustomerProjectSequence customerProjectSeq = sequenceDAO.findProjectCodeSequenceByCustomerId(customerId);
		if (!ObjectUtil.isEmpty(customerProjectSeq)) {
			if (!StringUtil.isEmpty(customerProjectSeq.getProjectCodeSeq())) {
				Integer updatedId = Integer.parseInt(customerProjectSeq.getProjectCodeSeq()) + 1;
				String updatedSequence = StringUtil.getExact(String.valueOf(updatedId), CUSTOMER_PROJECT_CODE_LENGTH);

				customerProjectSeq.setProjectCodeSeq(updatedSequence);
				clientDAO.update(customerProjectSeq);
			}

		} else {

			customerProjectSeq = new CustomerProjectSequence();
			customerProjectSeq.setCustomerId(customerId);
			if (!StringUtil.isEmpty(customerProjectCodeSeq)) {
				customerProjectSeq.setProjectCodeSeq(customerProjectCodeSeq
						.substring(customerProjectCodeSeq.length() - CUSTOMER_PROJECT_CODE_LENGTH));
			}
			clientDAO.save(customerProjectSeq);
	...
	public String getFarmCropsMasterCodeSeq() {
		Long sequence = sequenceDAO.getFarmCropsMasterCodeSeq();
		String farmCropsMasterCodeSequencePrex = getPrefix(sequence.toString().length(),
		                FARM_CROPS_MASTER_CODE_PREFIX_LENGTH);
		return FARM_CROPS_MASTER_CODE_PREFIX_STRING + farmCropsMasterCodeSequencePrex + sequence;
	}
