...
@Component
public class ProductReturnFromFarmer implements ITxnAdapter {

    private static final Logger LOGGER = Logger.getLogger(ProductReturnFromFarmer.class.getName());
    ...
    public Map<?, ?> process(Map<?, ?> reqData) {
        ...
        if (ESETxn.ONLINE_MODE == Integer.parseInt(head.getMode())) {
            ...
            ProductReturn exist = productDistributionService.findProductReturnByReceiptNoTxnType(receiptNo, txnType);
            if (!ObjectUtil.isEmpty(exist)) {
                throw new SwitchException(
                        TransactionTypeProperties.PRODUCT_RETURN_FROM_FARMER.equalsIgnoreCase(txnType)
            ...
            if (TransactionTypeProperties.PRODUCT_RETURN_FROM_FARMER.equalsIgnoreCase(txnType)
           ...
           /* if(!StringUtil.isEmpty(batchNo)){
            ...
            OfflineProductReturn offlineProductReturn = new OfflineProductReturn();
            offlineProductReturn.setTxnType(txnType);
            offlineProductReturn.setReceiptNo(offlineReceiptNo);
            offlineProductReturn.setProductReturnDate(
                    (String) reqData.get(TxnEnrollmentProperties.PRODUCT_DISTRIBUTION_DATE));
            offlineProductReturn
            ...
            offlineProductReturn.setVillageCode(
            ...
            offlineProductReturn.setAgentId(agentId);
            offlineProductReturn.setDeviceId(serialNo);
            offlineProductReturn.setServicePointId(!ObjectUtil.isEmpty(agent)
                    ? String.valueOf(agent.getServicePoint().getId()) : null);
            offlineProductReturn.setFreeDistribution(isFreeDistribution);
            if ("0".equalsIgnoreCase(offlineProductReturn.getFreeDistribution())) {
            	offlineProductReturn.setTax((StringUtil.isEmpty(tax)) ? 0.00 : Double.valueOf(tax));
            	offlineProductReturn
                        .setPaymentMode(StringUtil.isEmpty(modeOfPayment) ? "" : modeOfPayment);
                // Adding payment amt
            	offlineProductReturn.setPaymentAmt(paymentAmt);

            } else if ("1".equalsIgnoreCase(offlineProductReturn.getFreeDistribution())) {
            	offlineProductReturn.setPaymentMode("");
            	offlineProductReturn.setTax(0.00);
            	offlineProductReturn.setTotalAmount(0.00);
                offlineProductReturn.setPaymentAmt("");
            }
            // Adding payment amt
            // offlineDistribution.setPaymentAmt(paymentAmt);
            // sets mobile number for unregistered farmer
            if (!StringUtil.isEmpty(mobileNumber))
            	offlineProductReturn.setMobileNumber(mobileNumber);

            offlineProductReturn.setStatusCode(ESETxnStatus.PENDING.ordinal());
            offlineProductReturn.setStatusMsg(ESETxnStatus.PENDING.toString());
            offlineProductReturn.setBranchId(head.getBranchId());
            offlineProductReturn.setCurrentSeasonCode((String)reqData.get(TxnEnrollmentProperties.CURRENT_SEASON_CODE));
            offlineProductReturn.setStockType(ProductReturn.MOBILE_STOCK);
            ...
            List<com.sourcetrace.eses.txn.schema.Object> offlineProductObject = offlineCollection
                    .getObject();
            Set<OfflineProductReturnDetail> offlineProductReturnDetailList = new HashSet<OfflineProductReturnDetail>();
            for (com.sourcetrace.eses.txn.schema.Object object : offlineProductObject) {
                List<Data> offlineProductData = object.getData();
                OfflineProductReturnDetail offlineProductReturnDetail = new OfflineProductReturnDetail();
                for (Data data : offlineProductData) {
                    String key = data.getKey();
                    String value = data.getValue();

                    if (TxnEnrollmentProperties.PRODUCT_CODE.equalsIgnoreCase(key)) {
                    	offlineProductReturnDetail.setProductCode(value);

                    }

                    if (TxnEnrollmentProperties.QUANTITY.equalsIgnoreCase(key)) {
                        offlineProductReturnDetail.setQuantity(value);
                    }

                    if (TxnEnrollmentProperties.PRICE_PER_UNIT.equalsIgnoreCase(key)) {
                        offlineProductReturnDetail.setSellingPrice(
                                (StringUtil.isEmpty(value)) ? 0.00 : Double.valueOf(value));
                    }
                    
                    if(TxnEnrollmentProperties.BATCH_NO.equalsIgnoreCase(key)){
                        offlineProductReturnDetail.setBatchNo(value);
                    }
                    
                   /* if ("0".equalsIgnoreCase(offlineProductReturn.getFreeDistribution())) {*/
                        if (TxnEnrollmentProperties.SUB_TOTAL.equalsIgnoreCase(key)) {
                            offlineProductReturnDetail.setSubTotal(value);
                        }
                   /* } else {
                    	offlineProductReturnDetail.setSubTotal("0.00");
                    }*/

                }
                offlineProductReturnDetail.setOfflineProductReturn(offlineProductReturn);
                offlineProductReturnDetailList.add(offlineProductReturnDetail);
            }

            double totalAmount = 0;
            offlineProductReturn.setOfflineProductReturnDetail(offlineProductReturnDetailList);
            for (OfflineProductReturnDetail detail : offlineProductReturnDetailList) {
                totalAmount = totalAmount + Double.valueOf(detail.getSubTotal());
            }

            double amountWithTax = 0;
            if (!StringUtil.isEmpty(offlineProductReturn.getTax())) {
                amountWithTax = (offlineProductReturn.getTax() / 100 * totalAmount) + totalAmount;
                offlineProductReturn.setTotalAmount(amountWithTax);
            } else {
            	offlineProductReturn.setTotalAmount(totalAmount);
            }
			if (TransactionTypeProperties.PRODUCT_RETURN_FROM_FARMER.equalsIgnoreCase(txnType)) {
				...
				offlineProductReturn.setPmtImageDetail(distImageSet);
			}
            /** SAVING OFFLINE PRODUCT RETURN AND PRODUCT RETURN DETAIL OBJECT **/
            try {
            	offlineProductReturn.setOfflineProductReturnDetail(offlineProductReturnDetailList);
                productDistributionService.saveOfflineProductReturn(offlineProductReturn);
            } catch (Exception e) {
                e.printStackTrace();
